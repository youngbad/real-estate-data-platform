...
def generate_mock_data(source_name: str, count: int) -> List[Dict[str, Any]]:
    ...
    districts = ["Centrum", "Mokotow", "Praga", "Wola", "Ochota", "Wrzeszcz", "Oliwa", "Stare Miasto", "Krzyki", "Srodmiescie"]
...
class OtodomScraper(BaseScraper):
    def parse_data(self, content: str) -> List[Dict[str, Any]]:
        logger.info("Extracting Otodom listings... (Generating 500 mock records for 2025)")
...
class GusDataFetcher(BaseScraper):
    ...
    def parse_data(self, content: str) -> List[Dict[str, Any]]:
        logger.info("Extracting GUS public data... (Generating 50 mock records for 2025)")
        return generate_mock_data("gus", count=50)


if __name__ == "__main__":
    # Use a persistent local data directory folder instead of /tmp 
    OUTPUT_DIRECTORY = os.getenv("RAW_DATA_DIR", os.path.join(os.getcwd(), "data", "raw", "json"))
    
    otodom = OtodomScraper(base_url="https://www.otodom.pl/pl/oferty/sprzedaz/mieszkanie", output_dir=OUTPUT_DIRECTORY)
    otodom.run("otodom")
    
    olx = OlxScraper(base_url="https://www.olx.pl/nieruchomosci/mieszkania/sprzedaz/", output_dir=OUTPUT_DIRECTORY)
    olx.run("olx")
    
    gus = GusDataFetcher(base_url="https://api.stat.gov.pl/mock_endpoint", output_dir=OUTPUT_DIRECTORY)
    gus.run("gus")
