...
class PostgresDataLoader:
    ...
    def load_data(self, input_dir: str):
        ...
        dim_date = dim_date.drop(columns=["date_scraped"]).drop_duplicates(
            subset=["date_id"]
        )

        dim_date.to_sql(
            "dim_date", self.engine, if_exists="append", index=False, method="multi"
        )
        ...
        fact_listings = fact_listings.rename(
            columns={"listing_id": "listing_natural_key"}
        )
