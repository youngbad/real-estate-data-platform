...
class ParquetListingsTransformer:
    ...
    def transform(self, df: DataFrame) -> DataFrame:
        ...
        df_transformed = df_transformed.withColumn(
            "price_per_m2",
            F.round(F.col("price") / F.col("area"), 2)
        )
        ...
        logger.info(f"Output record count after transformations: {df_transformed.count()}")
...
if __name__ == "__main__":
    ...
    spark_session = SparkSession.builder \
        .appName("CurateRealEstateParquet") \
    ...
    INPUT_DIR = os.getenv("INPUT_PARQUET_DIR", os.path.join(os.getcwd(), "data", "processed", "parquet"))
    OUTPUT_DIR = os.getenv("OUTPUT_PARQUET_DIR", os.path.join(os.getcwd(), "data", "curated", "parquet"))
